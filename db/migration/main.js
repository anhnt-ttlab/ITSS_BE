...
con.connect(function (err) {
  ...
  var sql =
    "CREATE TABLE lessons (lesson_id INT AUTO_INCREMENT, lesson_name VARCHAR(255), time VARCHAR(255), course_id INT, PRIMARY KEY (lesson_id), FOREIGN KEY (course_id) REFERENCES courses(course_id) ON DELETE CASCADE)";
  con.query(sql, function (err, result) {
    if (err) throw err;
